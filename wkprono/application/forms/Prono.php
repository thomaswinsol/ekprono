...
class Application_Form_Prono extends My_Form  {
		...
		public function isValid($data){   	
    		...
    		if ($valid) {
    			...
    			for ($ii=51; $ii<=51; $ii++) {
    				if (isset($arr[$data[$ii."_1"]]) ) {
    					$this->getElement('naam')->addError('Troostfinale/Finale: Fouten ->wedstrijd '.$ii."-".$ploegen[$data[$ii."_1"]]. " reeds geselecteerd.");
    					$valid=false;
    					break;
    				}
				if (isset($arr[$data[$ii."_2"]]) ) {
    					$this->getElement('naam')->addError('Troostfinale/Finale: Fouten ->wedstrijd '.$ii."-".$ploegen[$data[$ii."_2"]]. " reeds geselecteerd.");
    					$valid=false;
    					break;
    				}
    				$arr[$data[$ii."_1"]]=1;
